...
var client_id : String = "n7hxvyomv184pzjp9c531qak3yalxg"
var client_secret : String = "m322dxegl1rf6qv7u7gazaa366hits"
var username : String = ""
var user_id : String = ""
...
#username of target twitch channel to join
...
var tts_voice_choice_index : int = 0


const USER_AGENT = "User-Agent: Fluffy Fox Bot powered by GIFT/4.0.0 (Godot Engine)"
	...
	while(peer.get_status() == peer.STATUS_CONNECTED):
		...
		if (peer.get_available_bytes() > 0):
			...
			if (data.has("error")):
				...
				var body := "Fluffy Fox Bot - User Token generated successfully! You can safely close this window now.".to_utf8_buffer()
				peer.put_utf8_string("HTTP/1.1 %d\r\n" % response_code)
				peer.put_utf8_string("Content-Length: %d\r\n\r\n" % body.size())
				peer.put_data(body)
				peer.disconnect_from_host()
...
func add_command(cmd_name : String, callable : Callable, max_args : int = 0, min_args : int = 0, permission_level : int = PermissionFlag.EVERYONE, where : int = WhereFlag.CHAT,  sound_file : String = "", default_command_volume : int = 70) -> void:
	commands[cmd_name] = CommandData.new(callable, permission_level, max_args, min_args, where, sound_file, default_command_volume)
		...
		if(cmd_data):
			...
			if(cmd_data.permission_level != 0):
				...
				if(user_perm_flags & cmd_data.permission_level == 0):
					...
					return
			if(arg_ary.size() == 0):
				cmd_data.func_ref.call(CommandInfo.new(sender_data, command, whisper,  cmd_data.sound_file, cmd_data.volume))
			else:
				cmd_data.func_ref.call(CommandInfo.new(sender_data, command, whisper, cmd_data.sound_file, cmd_data.volume), arg_ary)
...
func fetch_tts_voices()->Array:
	var voices : Array = DisplayServer.tts_get_voices_for_language("English")
	return voices

func change_target_channel(new_channel : String):
	target_channel = new_channel
	join_channel(target_channel)
	var channel_file : FileAccess = FileAccess.open("user://gift/channel", FileAccess.WRITE)
	channel_file.store_var(target_channel)
	channel_file.close()

func remove_token_file():
	if (FileAccess.file_exists("user://gift/auth/user_token")):
		var dir : DirAccess = DirAccess.open("user://gift/auth")
		dir.remove("user://gift/auth/user_token")

func call_capture_all_commands() -> Dictionary:
	var sound_command_dict : Dictionary = {}
	for command_name in commands:
		print("commands[command_name]: " , commands[command_name])
	for command_name in commands:
		var command_data : CommandData = commands[command_name]
		if command_data.sound_file !="":
			var cmd_dict : Dictionary = {"permission_level" : command_data.permission_level,
"sound_file" : command_data.sound_file,
"volume" : command_data.volume,
"max_args" : command_data.max_args,
"min_args" : command_data.min_args, 
"where" : command_data.where }
			sound_command_dict[command_name] =  cmd_dict
	return sound_command_dict
